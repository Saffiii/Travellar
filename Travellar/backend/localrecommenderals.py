...
def initLocalRecommenderALS(userid):
    # As ratings develop, can change the 'destinationCountry' to 'destinationState' or 'destinationName' in the dataframe
    ...
    user_list = ratings.userID.tolist()
    userset = set(user_list)
    ...
    result = print_recommendations(W, Q, Q_hat, destination_names, userset, userid)
    return result
...
def print_recommendations(W, Q, Q_hat, destination_names, userset, user=3):
    # print(user)
    # print(W) # 0/1 matrix
    ...
    for jj, destination_id in zip(userset, destination_ids):
        if jj == user:
            # print('User {} liked {}'.format(jj +1, ', '.join([destination_names[ii] for ii, qq in enumerate(Q[jj]) if qq > 3])))
            # print('User {} did not like {}'.format(jj +1, ', '.join([destination_names[ii] for ii, qq in enumerate(Q[jj]) if qq < 3 and qq != 0])))
            # print(' User {} recommended destination is {}'.format(jj, destination_names[destination_id]))
